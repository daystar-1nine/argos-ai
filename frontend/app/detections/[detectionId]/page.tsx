...
import LiveMLAnalyzer from '@/components/forensics/LiveMLAnalyzer';

export default function ForensicDetectionDetailPage() {
  const params = useParams();
  const detectionId = (params.detectionId as string) || 'match_01';

  // Toggle between real live ML neural analysis and archived forensic case
  const [activeTab, setActiveTab] = useState<'live_ml' | 'archived_case'>(
    detectionId === 'live' ? 'live_ml' : 'live_ml'
  );
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#F8E8E8] text-[#111111] font-mono">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <DashboardHeader title={`Forensics // ${detectionId === 'live' ? 'LIVE NEURAL ENGINE' : detectionId}`} />
        ...
        <div className="p-6 lg:p-8 space-y-8 flex-1 max-w-[1440px] w-full mx-auto">
          ...
          <PageHeader
            badge={activeTab === 'live_ml' ? "PROBLEM STATEMENT 4: REAL ML ENGINE" : "SYNCNET FORENSIC STUDIO"}
            badgeColor={activeTab === 'live_ml' ? "mauve" : "red"}
            title={activeTab === 'live_ml' ? "AUDIO-VISUAL TEMPORAL FORENSIC LAB" : `DETECTION CASE: ${detectionId.toUpperCase()}`}
            subtitle={
              activeTab === 'live_ml'
                ? "Execute real dual-stream SyncNet 3D-CNN & 2D-CNN feature extraction, continuous temporal correlation S(t), and multimodal deepfake classification on custom video streams."
                : `Target: ${analysis.assetId} • Multi-Model Consensus: 4/4 Models • Overall Risk: ${analysis.overallManipulationRisk}%`
            }
            actions={
              <div className="flex flex-wrap items-center gap-2">
                <button
                  onClick={() => setActiveTab('live_ml')}
                  className={`px-4 py-2 border-[2px] border-[#111111] text-xs font-black uppercase transition-all ${
                    activeTab === 'live_ml'
                ...
                <button
                  onClick={() => setActiveTab('archived_case')}
                  className={`px-4 py-2 border-[2px] border-[#111111] text-xs font-black uppercase transition-all ${
                    activeTab === 'archived_case'
                      ? 'bg-[#111111] text-[#F4CD3F] shadow-[2px_2px_0px_#844469]'
                      : 'bg-white hover:bg-[#F8E8E8] text-black'
                  }`}
                >
                  📁 ARCHIVED CASE EXHIBIT
                </button>
              </div>
            }
          />

          {/* TAB 1: Real PyTorch Multimodal ML Pipeline */}
          {activeTab === 'live_ml' && (
            <LiveMLAnalyzer />
          )}

          {/* TAB 2: Archived Case Exhibit & Forensic Comparison Feed */}
          {activeTab === 'archived_case' && (
            <div className="space-y-8">
              <div className="p-4 bg-[#EFD99C] border-[2px] border-[#111111] flex items-center justify-between text-xs">
                <span className="font-bold">
                  Viewing archived forensic audit for Case #{detectionId.toUpperCase()} (Target: {analysis.assetId}).
                </span>
                <Link
                  href="/incidents/ARG-8291"
                  className="px-3 py-1 bg-[#D95D5D] hover:bg-[#eb7373] text-white border border-[#111111] font-black uppercase"
                >
                  Open Incident ARG-8291 →
                </Link>
              </div>
